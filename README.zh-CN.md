...
角色卡导入先暂存文件，再将角色、内嵌世界书和挂载关系放在同一 SQLite 事务中提交。保存失败会回滚新记录并清理本次创建的文件，复用的世界书保持原状。导入上限为 JSON 16 MiB、PNG/CHARX 256 MiB、每本世界书 20,000 条目。流式回复按 120 毫秒合并普通显示更新，保留完整正文，并及时显示首段及完成状态。空首页提供配置模型和导入角色入口；接入商数量表示已启用连接，不代表聊天模型已配置。
