...
- UI 语言范围固定为简体中文、台湾繁体中文、English 和日本語；只翻译应用界面，不翻译模型 Prompt、角色卡、聊天正文、协议值或用户资料。全新数据根首次创建后必须完成一次语言选择；旧数据库缺少语言设置时静默保持简体中文。设置页语言变更在重启后生效。
...
- `LanguageRuntime` 负责文化名称归一化、资源字典装载、回退和设置键 `ui.language`；四份字典位于 `src/TavernDesk.App/Localization/Strings.*.xaml`。
- 全新数据根在数据库创建后写入 `.first-run-language.pending`，用户选定语言并保存后才删除；初始化中断时下次仍会继续选择。已有数据库没有设置且没有待选标记时不弹窗，使用简体中文。
- 首次选择在主窗口和 ViewModel 构造前应用；设置页只保存下次启动语言，不在运行中重建窗口树。
- 首次语言窗使用不透明标准 WPF 窗口和既有 `WindowBackgroundBrush`、`SurfaceSolidBrush`、`BorderBrush`、`TextBrush`、`MutedTextBrush`，不再透出桌面背景。
- 固定 UI 文本已从 App XAML/C# 剥离到语言字典。模型 Prompt、协议字面量、角色与用户内容保持原文；非当前语言的后端诊断在 App 展示边界转换为本地化摘要，原始内容只写入诊断跟踪。
- `scripts/Test-Localization.ps1` 检查四语键集合、空值、占位符、未完成标记、App 层简体硬编码、直接资源引用和 `depth/role/system/user/assistant` 协议值。
...
- 四种语言各 `1400` 个资源键，键集合、占位符、非空值、翻译标记、协议字面量和 `1553` 个直接 XAML 资源声明/引用检查通过；英文与简中完全相同的值只有五个协议字面量；
- Debug 与 Release 解决方案构建均为 `0` 警告、`0` 错误，`git diff --check` 通过；
- 新隔离目录执行 AgentHost `--storage-smoke` 为 `11` 项 PASS，API 请求数 `0`；
- 全新隔离数据根通过首次窗口选择 English 后进入主界面，语义自动化打开 Characters，进程内只有一个顶层窗口、无异常弹窗，随后正常退出；
- 最终 Release 复用该 English 隔离数据根进入 InputIdle，取得非零主窗口句柄并以退出码 `0` 正常关闭；
- 用户从实际 EXE 手动检查 English 界面，未发现问题。英文下拉框裁切已用紧凑标签、单行省略模板修复；Characters 连续弹窗根因是英文 `Characters.SelectedSuffix` 空资源，现已禁止任何空语言值并抑制重复未处理异常弹窗；
- 首次语言窗口的透明背景已改为不透明既有主题资源。第二次自动像素截图受 Windows UI Automation `RPC_E_SERVERFAULT` 影响未完成，因此台湾繁中、日语与首次窗口的最终像素级验收仍由人工完成；
- `win-x64` 自包含发布已同步到工作树 `app/`；三个业务 DLL 与 RID Release 输出的 SHA-256 一致，无 PDB，且只保留当前 RID 的原生 SQLite 运行库。根启动器 `TavernDesk.exe --probe` 退出码为 `0`。本次交付按用户授权在本地提交并快进合并到主工作树，不推送远端。
...
- 从重新发布后的根 EXE 对简体、台湾繁中、English、日语各做一次首次启动与设置重启短验收；目前 English 已由用户实际检查，其他三种尚无等价人工证据。
- 日语与台湾繁中的完整页面长文本换行、术语自然度和高 DPI 控件宽度；静态翻译审计与编译不能替代母语视觉反馈。
...
1. **按需补齐其余语言人工冒烟**
   - `app/` 已同步且根启动器探针为 0；English 已由用户实际检查。
   - 使用全新隔离数据根分别选择简体中文、繁體中文、日本語；检查首次窗背景、主导航、聊天下拉框、Characters、设置 → 界面，并验证设置语言重启后生效。
2. **再做两项既有人工冒烟**
