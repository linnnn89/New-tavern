...
当前数据库版本为 schema v22。维护者通常不需要记住每一列，只需先按职责定位：
...
- 群聊运行时接力固定按启用成员顺序推进；完全自动接力在上一条回复完成后继续，顶部成员头像的“立即接话”可强制指定角色。模型输出的 `@` 文本不会选择下一位角色，也不会触发用户暂停；旧的 `MentionDirected`、`Random` 和相关提示字段仅作为旧数据兼容项保留。
