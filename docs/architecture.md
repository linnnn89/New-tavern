...
当前数据库版本为 schema v19。维护者通常不需要记住每一列，只需先按职责定位：
...
| 记忆与群聊 | 记忆正文、草稿、检查点、群聊设置/成员/状态 | 更新先形成草稿；只有明确提交才推进相应检查点 |
...
- 角色、群聊和跑团分别拥有自己的记忆 owner，不共享实时正文。
- 普通记忆更新、压缩和群聊合并各只有一份全局职责模板；动态输入由代码构造，不恢复第二套 User 模板。
- 自动阈值更新只生成待确认草稿；用户保存后才覆盖正文并推进检查点。
- 群聊分支复制消息、候选、设置和成员，但不复制原群聊记忆，避免分支后信息泄漏。
