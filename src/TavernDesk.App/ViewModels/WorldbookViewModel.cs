...
public sealed class WorldbookMountListItem
{
    ...
    {
        Mount = mount;
        ScopeText = mount.ScopeKind == WorldbookScopeKind.Character
            ? $"角色：{(characterNames.TryGetValue(mount.ScopeId, out var name)
                ? name
                : mount.ScopeId)}"
            : mount.ScopeText;
    }
...
public sealed class WorldbookViewModel : ViewModelBase
{
    ...
    private async Task LoadSelectedBookAsync(string worldbookId)
    {
        try
        {
            ...
            {
                Mounts.Add(new WorldbookMountListItem(mount, characterNames));
            }
