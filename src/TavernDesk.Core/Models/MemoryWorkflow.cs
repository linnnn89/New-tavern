...
public sealed class MemoryWorkflowSettings
{
    ...
    // New memory banks enable threshold-based memory updates by default. Those
    // automatic updates are committed; manually generated updates remain
    // reviewable drafts until the user saves them.
...
{
    public const string NarrativeContract =
        """
        【记忆叙述契约】
        记忆正文是供后续对话检索的中立事实记录，不是任何角色的内心独白，也不是用户视角日志。
        - 默认使用明确实体名称或“用户”的第三人称表述：例如“用户答应明天回来”“角色甲等待用户”，不要把来源中的“我”直接写成记忆正文中的“我”。
        - 【记忆主体】表示这份记忆归属谁，不表示正文的叙述者；【用户身份】表示来源中用户发言的实际称呼。来源标签、USER、我、你和我们都只是资料中的指代，必须结合说话者和上下文重新绑定。
        - 每条事实尽量写清行动者、对象、结果和状态；无法确定指代时保留不确定性，不猜测、不把角色台词改写成客观事实。
        - 除非必须保留原话，否则不要使用第一人称或第二人称。必须保留原话时，用引号并注明说话者。
        - 旧记忆已有第一人称时，在输入能够确认主体的范围内改写为实体名称或“用户”；无法确认时不要擅自改变事实归属。
        """;
