...
public sealed class SqliteWorldbookRepository : IWorldbookRepository
{
    ...
    public SqliteWorldbookRepository(SqliteDatabase database)
    {
        _database = database;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await using (var book = connection.CreateCommand())
            {
                ...
                book.Parameters.AddWithValue("$sourcePath", worldbook.SourcePath);
    ...
    private static Worldbook ReadWorldbook(SqliteDataReader reader) =>
        new()
        {
            Id = reader.GetString(0),
