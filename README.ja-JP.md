...
キャラクターカードのインポートはファイルを一時保存し、キャラクター、内蔵ワールドブック、マウントを同じ SQLite トランザクションで確定します。保存に失敗すると新規レコードと今回作成したファイルを取り消し、再利用したワールドブックは維持します。上限は JSON 16 MiB、PNG/CHARX 256 MiB、ワールドブック当たり 20,000 エントリーです。ストリーミングの通常表示更新は 120 ミリ秒単位でまとめ、全文を保持し、最初の本文と完了状態を速やかに表示します。空のホーム画面にはモデル設定とカードインポートへの入口があり、接続先の件数は有効な接続を表し、チャットモデルの設定済み件数ではありません。
