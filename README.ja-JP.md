...
プレイヤーには、リポジトリのルートにある `TavernDesk-Setup-x64.exe` インストーラーの使用を推奨します：
...
インストーラーにはプライベート .NET 10 ランタイムと必要な依存関係がすべて含まれています。レジストリエントリは作成しないため、Windows の「インストールされているアプリ」には表示されません。スタートメニューのアンインストールショートカット、またはインストール先の `Uninstall TavernDesk.cmd` を使用してください。
...
プライバシーに配慮したローテーション式エラーログは、既定で `%LOCALAPPDATA%\TavernDesk\logs` に保存されます。エラー分類、例外の種類、マスク済みの呼び出し位置、状態だけを含み、API のリクエスト/応答本文や API Key は含みません。設定の API テストモードは既定でオフです。有効にすると、リクエスト本文、表示される応答、所要時間、Token 使用量をアプリケーションルートの `tests\output` に保存します。画面には会話内容が含まれることを明示し、フォルダーを直接開くかクリアできます。API Key、認証ヘッダー、Cookie、非表示の思考テキスト、完全な Embedding ベクトルは常に除外されます。インストール版のテスト出力はアンインストール時にアプリとともに削除されます。
