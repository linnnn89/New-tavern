...
- シナリオ本文とワールドブックのマウントをまとめて保存。保存に失敗しても下書きを保持して再試行でき、ライブラリに戻ると未保存の変更を破棄。

### Windows デスクトップ体験

- Windows 10/11 x64 向けのネイティブ WPF UI。
- 折りたためるコンテキストインスペクターを備えた 4 カラムのチャット画面。
- ライト/チャコールダークテーマ、表示倍率、共通フォント設定。 テーマと倍率は選択するとすぐにプレビューされます。保存すると表示設定を永続化し、フォントとチャットの自動スクロールを適用します。言語は保存して再起動した後に反映されます。既定値に戻した場合も保存が必要です。
