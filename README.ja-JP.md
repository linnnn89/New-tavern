...
- **送信内容を確認可能。** コンテキストインスペクターで Token 推定、リクエストの各区間、ワールドブックの一致結果、検索診断、除外項目、API リクエスト構造を確認できます。通常チャットのプレビューは現在の入力と会話に追従し、遅れて返った結果が新しいプレビューを上書きすることはありません。送信用リクエストの組み立て後は、入力の編集または会話の切り替えまで、そのトークン予算を優先表示します。
...
- シナリオ本文とワールドブックのマウントを同一トランザクションで保存します。編集は毎秒ローカルに保持し、通常終了前に最後の変更を書き込みます。キャンペーンを再度開くと復元または破棄を選べます。確認画面を閉じた場合は下書きを保持します。保存成功時は同じトランザクションで下書きを削除し、ライブラリに戻る場合は編集を破棄します。元のシナリオが変更・削除されていれば、新しいシナリオとして復元します。異常終了時は最後に書き込みが完了した後の編集が失われる可能性があります。

### Windows デスクトップ体験

- Windows 10/11 x64 向けのネイティブ WPF UI。
- 折りたためるコンテキストインスペクターを備えた 4 カラムのチャット画面。
- 新しいワークスペースの表示倍率は 100% です。倍率変更時はアプリ内倍率に影響されない最前面の確認画面を表示します。10 秒以内に確認するとその倍率を保存し、時間切れや画面を閉じた場合は直前の倍率へ戻ります。その他の設定や既定値へのリセットは「保存」が必要で、言語変更は再起動後に反映されます。既存の倍率設定は維持します。
- アプリ内倍率 100% を優先し、メインウィンドウの最小高さを 500 論理単位に設定。表示設定は説明と選択欄を縦に配置し、主要なシナリオ入力欄、チャット入力欄、倍率選択にアクセシブル名を付与。長い履歴はバックグラウンドで読み取り、メッセージを分割して画面へ反映し、キャンセルと会話切り替えの保護を維持します。Windows の DPI とアプリ内倍率は別々に扱います。
...
設定で保存先を変更すると、次回起動時の切り替えが予約されます。実行中は現在のフォルダーを使い続け、次回の単一起動チェック後、業務サービスがデータベースを開く前にコピーするため、予約後の更新も含まれます。コピーや設定の保存に失敗した場合は元の保存先を使い、成功後も元のフォルダーを残します。現在の保存先を再度保存すると予約を取り消せます。中断したコピーの再試行では、既存のコピーが保存先と同じ階層の `.name.incomplete-<id>` に保管される場合があります。

キャラクターカードのインポートはファイルを一時保存し、キャラクター、内蔵ワールドブック、マウントを同じ SQLite トランザクションで確定します。保存に失敗すると新規レコードと今回作成したファイルを取り消し、再利用したワールドブックは維持します。上限は JSON 16 MiB、PNG/CHARX 256 MiB、ワールドブック当たり 20,000 エントリーです。ストリーミングの通常表示更新は 120 ミリ秒単位でまとめ、必要な時点で全文スナップショットを作成し、変更のない本文とコードの表示を再利用します。最初の本文と完了状態を速やかに表示し、全文と既存の Markdown 構文を保持します。空のホーム画面にはモデル設定とカードインポートへの入口があり、接続先の件数は有効な接続を表し、チャットモデルの設定済み件数ではありません。
