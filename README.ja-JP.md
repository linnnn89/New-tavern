...
インストーラーにはプライベート .NET 10 ランタイムと必要な依存関係がすべて含まれています。レジストリエントリは作成しないため、Windows の「インストールされているアプリ」には表示されません。スタートメニューのアンインストールショートカット、またはインストール先の `Uninstall TavernDesk.cmd` を使用してください。アンインストールではセットアップが管理するプログラムファイルと `tests\output` だけを削除し、後からインストール先へ置いたその他のファイルは残します。
