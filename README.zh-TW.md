...
角色卡匯入先暫存檔案，再將角色、內嵌世界書和掛載關係放在同一 SQLite 交易中提交。儲存失敗會回復新記錄並清理本次建立的檔案，重用的世界書保持原狀。匯入上限為 JSON 16 MiB、PNG/CHARX 256 MiB、每本世界書 20,000 條目。串流回覆每 120 毫秒合併一般顯示更新，保留完整正文，並及時顯示首段及完成狀態。空白首頁提供設定模型和匯入角色入口；供應商數量表示已啟用連線，不代表聊天模型已設定。
