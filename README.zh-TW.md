...
- **上下文可以核對。** 檢查器會顯示 Token 估算、請求分段、世界書命中、檢索診斷、排除項目和實際 API 請求結構。一般聊天預覽跟隨目前輸入和對話，較晚返回的結果不會覆蓋新預覽；請求組裝完成後優先顯示傳送預算，直到你修改輸入或切換對話。
...
- 群聊接力預設按成員固定順序進行。完全自動接力會在上一則回覆完成後繼續；點擊上方成員頭像的「立即接話」可以強制指定角色。模型回覆中的 `@` 文字不會選擇下一位角色，也不會暫停接力。
...
- 劇本內容與世界書掛載在同一交易中儲存。編輯每秒保留到本機，正常退出前補寫；再次進入跑團時可復原或明確捨棄，關閉復原提示則保留到下次。儲存成功會在同一交易內清除復原草稿，返回劇本庫則放棄目前修改。原劇本已變更或被刪除時，復原內容另存為新劇本。異常終止仍可能遺失最後一次成功寫入之後的編輯。

### Windows 桌面體驗

- 原生 WPF 介面，支援 Windows 10/11 x64。
- 四欄聊天工作區，右側上下文檢查器可收合。
- 新資料庫預設使用 100% 應用程式縮放。切換比例時顯示不受應用程式縮放影響的置頂確認視窗：10 秒內確認即儲存該比例，逾時或關閉視窗會自動還原先前比例。其他介面偏好及還原預設後的設定仍需點擊儲存，語言變更需重新啟動；既有縮放設定繼續保留。
- 優先保障應用程式 100%：主視窗最小高度為 500 個邏輯單位，介面設定的標題與選項上下排列，關鍵劇本輸入欄位、聊天輸入框和縮放控制項提供可存取名稱。長歷史讀取移至背景，訊息分批套用到介面，保留取消和會話切換保護。Windows 系統縮放與應用程式縮放分別處理。
...
安裝包內含私有 .NET 10 執行階段和全部必要依賴，不建立登錄項目，因此不會出現在 Windows「已安裝的應用程式」清單中。可使用開始功能表的解除安裝捷徑，或安裝目錄中的 `Uninstall TavernDesk.cmd` 解除安裝。升級和解除安裝都會刪除安裝程式管理的程式檔案和 `tests\output`；使用者之後放入安裝目錄的其他檔案會保留。
...
在設定中變更資料目錄，會安排於下次啟動時切換。本次執行繼續使用目前目錄；下次取得單一執行個體鎖定後、業務服務開啟資料庫前才開始複製，因此安排遷移後繼續產生的資料也會包含在內。複製或設定提交失敗時仍使用原目錄，成功複製後也保留原目錄。重新儲存目前目錄可取消待執行切換。重試中斷的複製時，已有副本可能保留為目標同層的 `.name.incomplete-<id>` 目錄。

角色卡匯入先暫存檔案，再將角色、內嵌世界書和掛載關係放在同一 SQLite 交易中提交。儲存失敗會回復新記錄並清理本次建立的檔案，重用的世界書保持原狀。匯入上限為 JSON 16 MiB、PNG/CHARX 256 MiB、每本世界書 20,000 條目。串流回覆每 120 毫秒合併一般顯示更新，按需產生全文快照，並在回覆增長時重用未變更的正文和程式碼控制項；首段及完成狀態即時顯示，保留完整正文和現有 Markdown 語法。空白首頁提供設定模型和匯入角色入口；供應商數量表示已啟用連線，不代表聊天模型已設定。
